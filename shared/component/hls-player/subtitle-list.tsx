...
export function SubtitleList({ subs, currentTime, containerRef, itemRefs, onSubtitleClick }: SubtitleListProps) {
  return (
    <div ref={containerRef} className="p-4 max-h-[400px] overflow-y-auto space-y-2 relative">
      {subs.map((sub, index) => {
        ...
        return (
          <div key={index} ref={(el) => { itemRefs.current[index] = el; }} className={isCurrentSub ? "flex text-primary" : "flex text-muted"}>
            <Button size="sm" onClick={() => onSubtitleClick(sub)} variant="ghost" className="rounded-full transition hover:scale-110">▶</Button>
            <div>
              {sub.content.split("\n").map((line, i) => (
      ...
      <div className="h-100 flex flex-col items-center justify-center text-center">--End-- <br />Thank you for watching!</div>
    </div>
  );
}
